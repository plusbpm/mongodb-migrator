...
function MongoDBMigrator (options) {
	this.db = null;
	if(options.host) {
		delete defaultOptions.mongourl;
	}
	this.opts = Object.assign({}, defaultOptions, options);

	this.opts.directory = path.join(process.cwd(), this.opts.directory);
...
MongoDBMigrator.prototype.connect = function () {
	...
	var mongourl = self.opts.mongourl;

	if(!mongourl) {
		mongourl = "mongodb://";
		if(self.opts.user) {
			mongourl += self.opts.user + ":" + self.opts.password + "@" 
		}
		mongourl += self.opts.host + ":" + self.opts.port + "/" + self.opts.db;
	}
...
MongoDBMigrator.prototype.create = function (name, after) {
	...
	if(typeof after !== "undefined") {
		if(!self.exists(after)) {
			return log("Dependence '"+ after +"' does not exist, \u001b[31m cancellation ... \u001b[39m");
		} else {
...
MongoDBMigrator.prototype.check = function () {
	...
	return self
		.connect()
		.then(function () {
			trackCollection = self.db.collection(self.opts.collection);
			...
			if(newMigrations.length) {
				log("There are new migrations - " + newMigrations.length);
			} else {
